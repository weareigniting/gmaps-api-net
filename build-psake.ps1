...
task Build -depends Clean,UpdateAssemblyInfoVersions { 
	...
	foreach ($build in $builds)
	{
		...
		if($config -eq $null) { $config = "Release" }
		...
		exec { msbuild /v:m "/t:Clean;Rebuild" "/p:Configuration=$config" (Get-BuildSettings($build)) "/p:Platform=Any CPU" "/p:OutputPath=$outputPath\" (GetConstants $build.Constants $sign) "$sourceDir\$name" } "Error building $name"
...
# LIBRARY packaging Optional build documentation, add files to final zip
...
{
	[hashtable]$ver = @{}
	$assemblyversion_master = "$sourceDir\AssemblyVersion_Master.cs"
	$assemblyversioninfo_pattern = 'AssemblyFileVersion\("?<V1>(([0-9]+)\.?){1,4}"\)'

	if( (Test-Path $assemblyVersion_master) -eq $true) {
		$matches = (Get-Content $assemblyversion_master) -match $assemblyversioninfo_pattern
		#$matches
		...
		Write-Host -ForegroundColor Yellow "Warning: No AssemblyVersion_Master was found. Using default version of 1.0.0"
		$ver.MajorMinor = "1.0"
		$ver.MajorMinorRevision = "1.0.0"
	}
	
	$ver.Build = Get-BuildTiming
	$ver.FullString = [String]::Concat($ver.MajorMinorRevision,".",[string]$ver.Build)
	
	Write-Verbose "Version numbers in effect:"
	Write-Verbose $ver
...
function Set-AssemblyInfoVersions(
    [CmdletBinding()]  
...
)
{
    ...
    begin {
        if($fileVersion -eq $null) { $fileVersion = $version }
        #$assemblyPattern = "[0-9]+(\.([0-9]+|\*)){1,3}"  
        $assemblyVersionPattern = 'Assembly(File)?Version\("([0-9]+(\.([0-9]+|\*)){1,3})"\)'  

    	$updatedAssemblyVersion = 'Assembly$1Version("' + $version.ToString() + '")'
    }
    
    process {  
        #$_.FullName
            (Get-Content $_.FullName) | ForEach-Object {  
                % { $_ -replace $assemblyVersionPattern, $updatedAssemblyVersion } |                  
            } | Set-Content $_.FullName
    }
}

#function Load-AssemblyInfoVersion(
#	[CmdletBinding()]
#)
#{
#}

function Get-BuildSettings($Project) {
#TODO more work here for integrating project settings with global properties
	
	$a = "/p:AssemblyOriginatorKeyFile=$signKeyPath" + " /p:SignAssembly=$sign" + " /p:TreatWarningsAsErrors=$treatWarningsAsErrors"
